...
taskQueue::taskQueue() {
  ...
  MPI_Win_create(taskQ, TASKQUEUE_SIZE, 1, MPI_INFO_NULL, 
	...
	curr_size = 0;
  communication::details::MPI_Exclusive_put((void*)&curr_size, 1, MPI_INT, id, SIZE_OFFSET,
                          						1, MPI_INT, taskQ_win);
  communication::details::MPI_Exclusive_put((void*)&curr_head, 1, MPI_INT, id, HEAD_OFFSET,
                          						1, MPI_INT, taskQ_win);	
  communication::details::MPI_Exclusive_put((void*)&curr_tail, 1, MPI_INT, id, TAIL_OFFSET,
                          						1, MPI_INT, taskQ_win);
};
...
bool taskQueue::enqueue(int dst_id, _stub *job) {
  ...
  oa << tw;
	...
	/*read list tail*/
  communication::details::MPI_Exclusive_get((void*)&curr_tail, 1, MPI_INT, dst_id, TAIL_OFFSET,
                        							1, MPI_INT, taskQ_win);
  communication::details::MPI_Exclusive_get((void*)&curr_size, 1, MPI_INT, dst_id, SIZE_OFFSET,
                        							1, MPI_INT, taskQ_win);
	if(curr_size >= MAX_QUEUE_SIZE) return false;
	/* add new task to list */
  communication::details::MPI_Exclusive_put((void*)(&oa.size()), 1, MPI_INT, dst_id, curr_tail,
                          1, MPI_INT, taskQ_win);
  communication::details::MPI_Exclusive_put(const_cast<void*>(oa.address()), oa.size(), MPI_PACKED, dst_id, 
													curr_tail+TASK_OFFSET, oa.size(), MPI_PACKED, taskQ_win);
	/* set new list tail */
	curr_tail += sizeof(int)+sizeof(_stub_wrapper);
	curr_size++;
  communication::details::MPI_Exclusive_put((void*)&curr_tail, 1, MPI_INT, dst_id, TAIL_OFFSET,
                          						1, MPI_INT, taskQ_win);
  communication::details::MPI_Exclusive_put((void*)&curr_size, 1, MPI_INT, dst_id, SIZE_OFFSET,
                          						1, MPI_INT, taskQ_win);
...
_stub *taskQueue::dequeue(int dst_id) {
  ...
  boost::mpi::packed_iarchive ia(MPI_COMM_WORLD);
	taskQ_lock->lock(dst_id);
	/*read list head*/
	communication::details::MPI_Exclusive_get((void*)&curr_head, 1, MPI_INT, dst_id, HEAD_OFFSET,
                        							1, MPI_INT, taskQ_win);
  communication::details::MPI_Exclusive_get((void*)&curr_size, 1, MPI_INT, dst_id, SIZE_OFFSET,
                        							1, MPI_INT, taskQ_win);
	if(curr_size == 0)
	...
	/* get next task */
  int count;
  communication::details::MPI_Exclusive_get((void*)&count, 1, MPI_INT, dst_id, curr_head,
                          						1, MPI_INT, taskQ_win);
  // Prepare input buffer and receive the message
	...
	*/
	ia.resize(count);
  communication::details::MPI_Exclusive_get(const_cast<void*>(ia.address()), ia.size(), MPI_PACKED, dst_id,
                          curr_head+TASK_OFFSET, ia.size(), MPI_PACKED, taskQ_win);
  _stub_wrapper tw;
  ia >> tw;
	/* set new list head */
	curr_head += sizeof(int)+sizeof(_stub_wrapper);
	curr_size--;
  communication::details::MPI_Exclusive_put((void*)&curr_head, 1, MPI_INT, dst_id, HEAD_OFFSET,
                          						1, MPI_INT, taskQ_win);
  communication::details::MPI_Exclusive_put((void*)&curr_size, 1, MPI_INT, dst_id, SIZE_OFFSET,
                          						1, MPI_INT, taskQ_win);
	taskQ_lock->unlock(dst_id);
